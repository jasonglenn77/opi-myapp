...
def update_opportunity(opp_id: int, body: OpportunityPatch, user=Depends(get_current_user)):
    ...
    with engine.begin() as conn:
        ...
        # Ownership + join model: documents are NOT moved on link/unlink. The
        # opportunity keeps its estimate folders (1–5); the linked project merges
        # them into its view at read time (see documents.routes). This keeps the
        # opportunity's doc count intact and makes unlinking non-destructive.
        row = conn.execute(text(_SELECT + " WHERE o.id = :id"), {"id": opp_id}).mappings().first()
    return {"opportunity": _row(row)}
