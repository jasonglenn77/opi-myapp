// Crew Offer + Payment Schedule panel — the project's "Payments" tab.
// Flow: office sends a work offer (labor + scope auto-filled from the accepted
// estimate) → crew accepts/declines (Crew Portal, or office records it) → on
// accept, the bi-weekly payment schedule is set up (even split, in arrears),
// editable, with actual paid amounts pulled from QuickBooks.
...
export async function mountPaymentsPanel(container, entityId) {
    ...
    if (latest && latest.status === "accepted") {
      ...
      inner = `<div class="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2.5">
        ...
        <div class="mt-2 flex items-center gap-2 flex-wrap">
          ...
          <span class="text-[10px] text-black/40">or the crew responds in the Crew Portal</span>
        </div>
      </div>`;
    } else {
      // send form (no active offer, or last was declined/withdrawn)
      ...
      inner = `<div class="rounded-xl border border-black/10 px-3 py-3">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <label class="block"><div class="text-[10px] font-bold uppercase tracking-wide text-black/40 mb-1">Labor offer ($)</div>
            ...
            <div class="text-[10px] text-black/40 mt-0.5">${suggested ? "From the accepted estimate's Contract Labor." : "No estimate labor found — enter the amount."}</div></label>
          <label class="block"><div class="text-[10px] font-bold uppercase tracking-wide text-black/40 mb-1">Scope of work</div>
            <input id="oScope" type="text" class="input text-sm py-1.5 w-full" value="${escapeHtml(offers.suggested_scope || "")}" placeholder="Scope from the estimate"></label>
        </div>
        <div class="mt-3 flex items-center gap-3">
          <button id="oSend" class="btn-primary text-sm px-4 py-2">Send offer</button>
          <span id="oMsg" class="text-xs font-semibold"></span>
        </div>
      </div>`;
    }
  ...
  function setupForm() {
    const e = data.entity, acc = offers.accepted;
    const labor = acc ? acc.labor_amount : (e.suggested_contract_labor || 0);
    const crew = acc ? acc.crew_id : e.suggested_crew_id;
    const src = acc ? "Prefilled from the accepted offer." : (labor ? "From the estimate's Contract Labor." : "");
    return `
      <div class="text-[11px] font-bold uppercase tracking-wide text-black/40 mb-2">Payment schedule</div>
      ${acc ? "" : `<div class="text-xs text-amber-700 mb-2">No accepted offer yet — you can still set up a schedule, or send/accept an offer above first.</div>`}
      <div class="max-w-2xl">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label class="block"><div class="text-[11px] font-bold uppercase tracking-wide text-black/40 mb-1">Contract labor ($)</div>
            <input id="pContract" type="number" step="100" class="input text-sm py-1.5 w-full" placeholder="0" value="${labor ? Math.round(labor) : ""}">
            <div class="text-[10px] text-black/40 mt-0.5">${src}</div></label>
          <label class="block"><div class="text-[11px] font-bold uppercase tracking-wide text-black/40 mb-1">Crew</div>
            <select id="pCrew" class="input text-sm py-1.5 w-full">${crewOptions(data.crews, crew)}</select></label>
  ...
  function scheduleView() {
    ...
    const qboRows = (qbo.payments || []).map(p => `<tr class="border-b border-black/5">
        ...
        <td class="py-1 px-2 text-right tabular-nums font-semibold text-kpi-completed-text">${money(p.amount)}</td></tr>`).join("");
    ...
    return `
      ...
      <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-3">
        <div class="flex items-end gap-3 flex-wrap">
          <label class="block"><div class="text-[10px] font-bold uppercase tracking-wide text-black/40 mb-1">Crew</div>
            <select id="pCrewEdit" class="input text-xs py-1.5">${crewOptions(data.crews, s.crew_id)}</select></label>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-xs table-fixed">${colgroup}
          ...
          : (qbo.payments || []).length
            ? `<div class="text-[11px] text-black/45 mb-1">Bill payments to this crew's vendor tagged to this project.</div>
               <div class="overflow-x-auto max-w-md"><table class="w-full text-xs">
                 ...
                 <tbody>${qboRows}<tr class="border-t border-black/10"><td class="py-1 px-2 font-bold">Total paid</td><td class="py-1 px-2 text-right font-extrabold tabular-nums">${money(paidQbo)}</td></tr></tbody></table></div>`
            : `<div class="text-xs text-black/45">No payments to this crew tagged to this project in QuickBooks yet.</div>`}
      </div>`;
  }

  const setMsg = (id, t, ok) => { const m = container.querySelector(id); if (m) { m.textContent = t; m.className = "text-xs font-semibold " + (ok ? "text-kpi-completed-text" : "text-red-600"); if (ok) setTimeout(() => { if (m.textContent === t) m.textContent = ""; }, 1500); } };

  function render() {
    container.innerHTML = `<div class="p-4 sm:p-5">${offerSection()}${data.schedule ? scheduleView() : setupForm()}</div>`;
    wire();
  }
  ...
  function wire() {
    ...
    if (send) send.addEventListener("click", async () => {
      ...
      if (!crew) { setMsg("#oMsg", "Select a crew.", false); return; }
      const body = { crew_id: Number(crew), labor_amount: parseFloat(container.querySelector("#oAmount").value) || 0, scope: container.querySelector("#oScope").value || null };
      send.disabled = true;
      try { await api(`/offers/project/${encodeURIComponent(entityId)}`, { method: "POST", body: JSON.stringify(body) }); await reload(); }
      catch (err) { setMsg("#oMsg", err.message, false); send.disabled = false; }
    });
    container.querySelectorAll("[data-respond]").forEach(b => b.addEventListener("click", async () => {
      ...
      try { await api(`/offers/${id}/respond`, { method: "POST", body: JSON.stringify({ status, note }) }); await reload(); }
      catch (err) { alert(err.message); }
    }));
    const wd = container.querySelector("[data-withdraw]");
    if (wd) wd.addEventListener("click", async () => {
      if (!confirm("Withdraw this offer so you can reassign to another crew?")) return;
      try { await api(`/offers/${wd.getAttribute("data-withdraw")}/withdraw`, { method: "POST" }); await reload(); }
      catch (err) { alert(err.message); }
    });
    ...
    if (gen) gen.addEventListener("click", async () => {
      ...
      if (!body.start_date || !body.end_date) { setMsg("#pMsg", "Start and end dates are required.", false); return; }
      gen.disabled = true;
      try { await api(`/payments/project/${encodeURIComponent(entityId)}/generate`, { method: "POST", body: JSON.stringify(body) }); await reload(); }
      catch (err) { setMsg("#pMsg", err.message, false); gen.disabled = false; }
    });
    ...
    if (crewEdit) crewEdit.addEventListener("change", async () => {
      ...
      catch (err) { setMsg("#pMsg", err.message, false); }
    });
    ...
    if (regen) regen.addEventListener("click", () => {
      ...
      data.schedule = null;   // show setup form
      render();
