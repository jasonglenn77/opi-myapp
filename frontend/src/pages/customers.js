...
export async function customersPage(routeFn) {
  ...
  let sortKey = "last_activity", sortDir = -1, search = "", groupBy = true;
  const filters = {};
  ...
  const statusBadge = (s) => s ? `<span class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold bg-black/5 text-black/60 truncate max-w-full">${escapeHtml(s)}</span>` : `<span class="text-black/25">—</span>`;
  ...
  const COLS = [
    ...
    { key: "status", label: "Status", type: "multi", w: 150, td: (r) => statusBadge(r.status) },
  ...
  const renderRows = () => {
    ...
    if (!tbody) return;
    if (!groupBy) {
      tbody.innerHTML = list.map(r => `<tr class="border-b border-black/5 hover:bg-black/[0.02]">${COLS.map(c => cell(r, c)).join("")}</tr>`).join("")
      ...
      tbody.innerHTML = order.map(cid => {
        ...
        const rows = open ? kids.map(r => `<tr class="border-b border-black/5 hover:bg-black/[0.02]"><td class="px-2"></td>${COLS.map(c2 => c2.key === "customer_name" ? `<td></td>` : cell(r, c2)).join("")}</tr>`).join("") : "";
        return header + rows;
        ...
        <div class="shrink-0 px-4 sm:px-5 pt-4 pb-3 border-b border-black/10">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            ...
            <div class="flex items-center gap-2 flex-wrap">
              <input id="custSearch" class="input text-xs py-1.5 w-full sm:w-56" placeholder="Search customer, name, status…" />
              <label class="flex items-center gap-1.5 text-xs text-black/60 whitespace-nowrap"><input type="checkbox" id="groupBy" class="h-4 w-4 rounded border-black/25" checked> Group by customer</label>
  ...
  document.getElementById("custSearch").addEventListener("input", (e) => { search = e.target.value; renderRows(); });
  document.getElementById("groupBy").addEventListener("change", (e) => { groupBy = e.target.checked; restampColgroup(); renderAll(); });
  ...
  document.getElementById("custScroll").addEventListener("scroll", closePortals);
